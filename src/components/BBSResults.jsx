  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, fontFamily: 'var(--font-mono)' }}>
        ...
        <tbody>
          {rows.map((r, i) => (
            <tr key={i} style={{ background: i % 2 === 0 ? 'var(--row-even)' : 'var(--row-odd)' }}>
              {showSource && (
                <td style={{ ...td, maxWidth: 140, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  <span style={{ fontSize: 11, color: 'var(--text2)' }}>{r.sourceLabel}</span>
                  {r.count > 1 && <span style={{ marginLeft:4, fontSize:10, color:'var(--primary)', fontWeight:700 }}>×{r.count}</span>}
                </td>
              )}
              <td style={{ ...td, textAlign: 'center' }}>
                <Badge label={r.mark} color="blue" />
              </td>
              <td style={{ ...td, color: 'var(--text)', textAlign: 'left', fontSize: 12 }}>{r.desc}</td>
              <td style={{ ...td, textAlign: 'center' }}>{r.nos}</td>
              <td style={{ ...td, textAlign: 'right' }}>{r.cutLen.toFixed(3)}</td>
              <td style={{ ...td, textAlign: 'right', color: 'var(--green)', fontWeight: 600 }}>{r.totalLen.toFixed(3)}</td>
              <td style={{ ...td, textAlign: 'right', color: 'var(--text2)' }}>{BAR_WEIGHT[r.dia] || '—'}</td>
              <td style={{ ...td, textAlign: 'right', color: 'var(--red)', fontWeight: 700 }}>{r.weight.toFixed(2)}</td>
            </tr>
          ))}
        </tbody>
        <tfoot>
          <tr style={{ background: '#deeefb', borderTop: '2px solid var(--primary)' }}>
    ...
    <div>
      {Object.entries(groups).map(([label, groupRows]) => (
        <div key={label} style={{ marginBottom: 16 }}>
          <div style={{ padding: '7px 14px', background: 'var(--primary-light)', borderLeft: '3px solid var(--primary)', marginBottom: 0, fontSize: 12, fontWeight: 700, color: 'var(--primary)' }}>
            {label} <span style={{ fontWeight: 400, color: 'var(--text3)', fontSize: 11 }}>— {groupRows[0]?.count} nos</span>
          </div>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, fontFamily: 'var(--font-mono)' }}>
          <thead>
            <tr style={{ background: '#f0f4f8' }}>
              {['Dia', 'Total Length', 'Weight (kg)', '12m Rods Reqd'].map(h => (
          ...
          <tbody>
            {costs.map((r, i) => (
              <tr key={r.dia} style={{ background: i%2===0?'white':'var(--row-odd)' }}>
                ...
                <td style={{ ...td, textAlign: 'center', color: 'var(--primary)', fontWeight: 700 }}>
                  <span style={{ background: 'var(--primary-light)', padding: '2px 8px', borderRadius: 4 }}>{r.rods12m} rods</span>
                </td>
              </tr>
            ))}
            <tr style={{ background: '#f0f4f8', borderTop:'2px solid var(--border)' }}>
              <td colSpan={2} style={{ ...td, fontWeight: 700 }}>TOTAL</td>
              <td style={{ ...td, textAlign:'right', fontWeight:700, color:'var(--primary-dark)' }}>{totalKg.toFixed(2)} kg</td>
              <td style={{ ...td, textAlign:'center', fontWeight:700, color:'var(--primary-dark)' }}>
      ...
      <div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, fontFamily: 'var(--font-mono)' }}>
          ...
          <tfoot>
            <tr style={{ background: '#eafaf1', borderTop: '2px solid var(--green)' }}>
              <td colSpan={3} style={{ ...td, fontWeight: 700, color: 'var(--green)', fontSize: 13 }}>TOTAL COST</td>
              <td style={{ ...td, textAlign: 'right', fontWeight: 800, color: 'var(--green)', fontSize: 18 }}>
...
const th = { padding: '8px 12px', border: '1px solid var(--border)', color: 'var(--primary)', fontWeight: 700, textAlign: 'center', whiteSpace: 'nowrap', fontSize: 11 };
