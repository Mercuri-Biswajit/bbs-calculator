...
export function DrawingFooting({ L, B, D, mainDia, distDia, spacing, hasStub, stubH, colDia, colNos }) {
  ...
  const pad = 65;
  const targetFw = 500;
  const scale = Math.min((targetFw - 2 * pad) / l, (400 - 2 * pad) / b, 300); // Added safety bound just in case
  const fw = l * scale,
    fh = b * scale;
  const W = Math.round(fw + 2 * pad);
  const H = Math.round(fh + 2 * pad);
  const ox = pad,
    oy = pad + 10; // push down for title
  ...
  // ── SECTION A-A (cut along B, showing L × D) ──
  const sPadX = 75;
  const sPadY = 120; // more room for stub col
  const targetSw = 500;
  ...
  const sox = sPadX,
    soy = sH - sPadY - sh + 20; // Anchor bottom
    ...
    <div>
      ...
      {view === "plan" ? (
        ...
        >
          ...
          {/* Dowels/Starters if stub is enabled */}
          {hasStub && (
            <g>
              <path
          ...
          {/* Main bars in section (circles at bottom, running in L direction) */}
          {secMainXs.map((x, i) => (
            <RebarDot
              key={i}
              cx={x}
          ...
          {/* Dist bars (horizontal lines at same level, running in B direction) */}
          <line
            x1={sox + scov}
            y1={soy + sh - scov}
            x2={sox + sw - scov}
