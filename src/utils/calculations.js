...
// ─── FOOTING ────────────────────────────────────────────────────────────────
export function calcSingleFooting({ L, B, mainDia, distDia, spacing, unit = 'm' }) {
...
// ─── COLUMN ─────────────────────────────────────────────────────────────────
export function calcSingleColumn({ H, B, D, mainDia, mainNos, tieDia, tieSpacing, unit = 'm' }) {
...
// ─── BEAM (plinth or wall) ───────────────────────────────────────────────────
export function calcSingleBeam({ L, B, D, botDia, botNos, topDia, topNos, exTopDia, exTopNos, stirDia, stirSpacing, unit = 'm' }, coverType) {
  const l = toMeters(L, unit);
  const b = toMeters(B, unit);
  const d = toMeters(D, unit);
...
// ─── SLAB (1-way or 2-way) ───────────────────────────────────────────────────
export function calcSingleSlab({ L, B, mainDia, distDia, mainSp, distSp, topDia, topSp, slabType = '2-way', unit = 'm' }) {
  const l = toMeters(L, unit);
  const b = toMeters(B, unit);
  ...
  const rows = [
    { mark:"A", desc:`Main Bars - ${is1Way ? 'Short span' : 'Both ways'} (φ${mainDia}mm @${mainSp}mm)`, nos: nMain, cutLen: +lenMain.toFixed(3), dia: +mainDia },
    { mark:"B", desc:`Dist Bars - ${is1Way ? 'Long span' : 'Both ways'} (φ${distDia}mm @${distSp}mm)`, nos: nDist, cutLen: +lenDist.toFixed(3), dia: +distDia },
  ];
...
export function calcSingleStaircase({ 
  ...
  return [
    { mark:"A", desc:`Main Bars along flight (φ${mainDia}mm @${mainSp}mm)`, nos: nMain, cutLen: +lenMain.toFixed(3), dia: +mainDia },
    { mark:"B", desc:`Distribution Bars (φ${distDia}mm @${distSp}mm)`, nos: nDist, cutLen: +lenDist.toFixed(3), dia: +distDia },
  ];
}
