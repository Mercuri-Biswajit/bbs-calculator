...
// FIX #1 — all 9 element types listed (lintel, raft, pileCap were missing)
const TABS_META = [
  ...
  { id: "lintel", icon: "[L]", label: "Lintel / Chajja" },
  { id: "raft", icon: "[RF]", label: "Raft Foundation" },
  { id: "pileCap", icon: "[PC]", label: "Pile Caps" },
];
...
function drawBBSSection(
  ...
  for (const [grpLabel, grp] of Object.entries(groups)) {
    ...
    doc.autoTable({
      ...
      showFoot: "lastPage",
      didDrawPage: (data) => {
        if (data.pageNumber > 1) drawPageHeader(doc, details, pageW, null);
      },
    });
...
function drawCombinedSummary(doc, costs, pageW, y) {
  ...
  // FIX #5 — use "O" (diameter symbol) instead of "phi" for PDF-safe rendering
  const tableRows = costs.map((c) => [
    `\u00D8${c.dia}mm`, // Ø symbol — universally supported in Helvetica
    `${c.totalLen} m`,
    `${c.kg} kg`,
    `${c.rods12m} rods`,
    `Rs.${c.ratePerPiece}/pc`,
...
export function generatePDF(details, byType, allRows, costs) {
  ...
  // FIX #1 — iterate ALL byType entries; look up meta safely with fallback
  byType.forEach((t) => {
    if (!t.rows.length) return;
    const meta = TABS_META.find((m) => m.id === t.type) || {
      icon: `[${t.type.toUpperCase().slice(0, 2)}]`,
      label: t.type,
    };
    if (y > pageH - 50) {
      doc.addPage();
